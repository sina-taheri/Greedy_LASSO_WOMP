...
addpath C:\Users\sinat\Dropbox\Shared_Sina_Simone\Numerics\WOMP_Sina_numerics\WOMP_paper_numerics\data\Fig_5
addpath C:\Users\sinat\Dropbox\Shared_Sina_Simone\Numerics\WOMP_Sina_numerics\WOMP_paper_numerics\src\utils

womp_type = 'srlassol1';
switch womp_type
    case 'lassol1'
        load('Fig_5_13_lassol1_d_10_N_471_m_200_n_iter_250');
        title_txt = '$\ell^1$-based LASSO-WOMP';
        im_save_txt = 'C:\\Users\\sinat\\Dropbox\\Shared_Sina_Simone\\Numerics\\WOMP_Sina_numerics\\WOMP_paper_numerics\\figs\Fig_5\\Fig_5_13_lassol1_d_10_N_471_m_200_n_iter_250';
        % extract a portion of lambdas (if required)
%         [~, idx] = intersect(lambda_vals, [0, 10^-5, 10^-4.5, 10^-3, 10^-2, 10^-1], 'stable');
%         lambda_vals = lambda_vals([idx', end]);
%         lambda_err = lambda_err(:, [idx', end], :);
%         n_plot = numel(lambda_vals);
    case 'srlassol1'
        load('Fig_5_23_srlassol1_d_10_N_471_m_200_n_iter_250');
        title_txt = '$\ell^1$-based SR-LASSO-WOMP';
        im_save_txt = 'C:\\Users\\sinat\\Dropbox\\Shared_Sina_Simone\\Numerics\\WOMP_Sina_numerics\\WOMP_paper_numerics\\figs\Fig_5\\Fig_5_23_srlassol1_d_10_N_471_m_200_n_iter_250';
        % extract a portion of lambdas (if required)
%         [~, idx] = intersect(lambda_vals, [0, 10^-8, 10^-2, 10^-2.5, 10^-1.75, 1], 'stable');
%         lambda_vals = lambda_vals([idx', end]);
%         lambda_err = lambda_err(:, [idx', end], :);
%         n_plot = numel(lambda_vals);
    case 'ladlassol1'
        load('Fig_5_33_ladlassol1_d_10_N_471_m_200_n_iter_250');
        title_txt = '$\ell^1$-based LAD-LASSO-WOMP';
        im_save_txt = 'C:\\Users\\sinat\\Dropbox\\Shared_Sina_Simone\\Numerics\\WOMP_Sina_numerics\\WOMP_paper_numerics\\figs\Fig_5\\Fig_5_33_ladlassol1_d_10_N_471_m_200_n_iter_250';
        % extract a portion of lambdas (if required)
...
% params structure
params = set_plot_params(5, title_txt);
...
ylabel('$ Relative-L_\varrho^2(\mathcal{U})\ error $', 'Interpreter', 'Latex', 'Fontsize', 20, 'FontWeight', 'bold'); % ylim([0.03 3]);
